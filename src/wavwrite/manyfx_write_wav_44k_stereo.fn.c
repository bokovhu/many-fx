    ...
    const char *filename)
{
    ...
    for (unsigned int i = 0; i < audio->sample_count; i++) {
        short leftSample = audio->left_data[i] * 32767; // convert from float [-1, 1] to 16-bit PCM
        fwrite(&leftSample, 2, 1, file);
        short rightSample = audio->right_data[i] * 32767; // convert from float [-1, 1] to 16-bit PCM
